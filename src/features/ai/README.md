...
`fetchAIPlanPreview(requestData)`
  - Method: POST
...
`generateTasksWithAI(requestData)`
  - Method: POST
...
All AI routes are protected. Browser clients must rely on the httpOnly cookie `jwt` set by the server; use a central `apiClient` configured with `withCredentials: true`. For non-browser clients, send `Cookie: jwt=YOUR_TOKEN` or an Authorization header if necessary.
